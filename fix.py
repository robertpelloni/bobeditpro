...
target = """    float strength = s.healStrength / 100.0f;
...
replacement = """    float strength = s.healStrength / 100.0f;
    ...
    if (lowBin > 0 && highBin < WindowSize / 2 && highBin > lowBin) {
        ...
        while (rightPhase - leftPhase > M_PI) rightPhase -= 2.0f * M_PI;
        while (leftPhase - rightPhase > M_PI) rightPhase += 2.0f * M_PI;
