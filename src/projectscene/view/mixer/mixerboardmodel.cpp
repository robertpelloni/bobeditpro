...
// Collect Busses
    ...
    {
        if (row < 0 || row >= m_strips.count()) {
            return;
        }
        auto& strip = m_strips[row];
        if (qFuzzyCompare(strip.volume, volume)) {
            return;
        }
    ...
    void MixerBoardModel::setPan(int row, double pan)
    {
        ...
        auto& strip = m_strips[row];
        if (qFuzzyCompare(strip.pan, pan)) {
            return;
        }
    ...
    void MixerBoardModel::setRoute(int row, int routeIndex)
    {
        ...
        if (routeIndex < 0 || routeIndex >= m_availableRouteIds.count()) {
            return;
        }

        int newRouteId = m_availableRouteIds[routeIndex];
        auto& strip = m_strips[row];
        if (strip.routeId == newRouteId) {
            return;
        }
    ...
    void MixerBoardModel::addSend(int row, int routeIndex)
    {
        ...
        // Cycle check!
        auto strip = m_strips[row];
        if (!canRoute(strip.trackId, destId)) {
            return;
        }
    ...
    bool MixerBoardModel::canRoute(int sourceId, int targetId) const
    {
        ...
        if (targetId == 0) {
            return true;        // Master is always safe
        }
        auto prj = globalContext()->currentProject();
        if (!prj) {
            return false;
